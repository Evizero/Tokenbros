async(page)=>{
 ...
 try{
  ...
  if(!await page.evaluate(()=>window.__qa.shotCount>0&&window.__qa.marcusKit.mode===2))throw Error('Mouse launch / mode input failed');
  ...
  const tests=await page.evaluate(()=>{
   ...
   let k=setup(),e=g.spawnEnemy(260,830,'gunner',-1);e.hp=30;g.enemies=[e];k.fire();ticks(k,60);assert(k.hitCount===1,'one outward hit');k.recall();ticks(k,180);assert(k.returnHits===1,'one return hit');assert(e.hp===22,'exactly one damage application per leg');assert(k.stock===3&&k.catches>0,'catch refunds stock');results.push('outbound + return hits and catch refund');
   k=setup();for(let y=37;y<43;y++)g.world.set(15,y,3);e=g.spawnEnemy(340,830,'gunner',-1);g.enemies=[e];k.fire();ticks(k,360);assert(e.hp===4,'no through-steel hits');assert(k.bounceCount>0,'bankshot collision');assert(k.stock===3,'blocked disc recovers stock');results.push('steel obstruction, ricochet, stock recovery');
   k=setup();k.fire();ticks(k,13);g.setThinking(1);assert(k.discs.every(d=>d.mode===0),'in-flight mode locked');k.recall();ticks(k,360);assert(k.stock===3,'discs returned after mode change');results.push('disc mode lock and recall after switching');
   k=setup(2);g.aimPoint=()=>({x:330,y:830});for(let y=41;y<43;y++)g.world.set(18,y,2);e=g.spawnEnemy(320,830,'shield',-1);g.enemies=[e];for(let i=0;i<170;i++)k.fireInput(1/120,true);k.fireInput(1/120,false);ticks(k,600);assert(g.world.destroyed>0,'invader bombs destroy cover');assert(e.dead||e.hp<7,'invader bombs damage shield guard');results.push('invader bombing cover and armor');
   ...
   k=setup();k.ultimate();assert(k.batch===4.5,'batch activates');k.fire();ticks(k,13);assert(k.discs.length===5&&k.stock===2,'bounded echoes do not cost slots');ticks(k,600);assert(k.batch===0&&k.stock===3,'batch and discs expire');k.ultimate();assert(k.batch===0,'batch cooldown');results.push('batch echoes and cooldown');
   k=setup();for(let i=0;i<3;i++){k.fire();ticks(k,13);}assert(k.stock===0,'three disc limit');const count=k.discs.length;k.fire();ticks(k,1);assert(k.discs.length===count,'cannot overspend stock');g.die();assert(k.discs.length===0&&k.pending===null,'death clears discs');g.respawn();assert(g.marcusKit.stock===3,'respawn restores discs');results.push('stock cap, death and respawn');

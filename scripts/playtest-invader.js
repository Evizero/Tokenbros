async(page)=>{
 ...
 const result=await page.evaluate(()=>{
  ...
  for(const [frames,count] of [[1,1],[80,3],[170,6]]){const k=setup();charge(k,frames);assert(k.invader.amount===count,'charge tier');assert(k.stock===2,'charge reserves one slot');const size=.75+k.invader.charge*4.1;k.fireInput(1/120,false);assert(k.invader.craft?.total===count,'launch payload');ticks(k,900);assert(k.invader.dropped===count,`payload dropped ${count}: ${k.invader.dropped}`);assert(k.invader.detonated===count,'finite explosions');assert(k.invader.caught===1&&k.stock===3,'returns / catches / refunds');results.push({count,size,caught:k.invader.caught});}
  let k=setup();charge(k,170);k.fireInput(1/120,false);ticks(k,20);k.recall();ticks(k,800);assert(k.invader.dropped===0,'recall cancels remaining bombs');assert(k.stock===3,'recall refund');results.push('early recall cancels bombing');
  k=setup();charge(k,170);k.fireInput(1/120,false);g.setThinking(0);k.fire();ticks(k,15);assert(k.discs.length>0&&k.invader.craft,'discs while bomber deployed');g.setThinking(2);charge(k,170);assert(!k.invader.charging,'one bomber at a time');results.push('mode switching and single bomber');
  k=setup();for(let x=5;x<35;x++)g.world.set(x,39,3);charge(k,170);k.fireInput(1/120,false);ticks(k,900);assert(k.invader.dropped===0&&k.stock===3,'ceiling blocks bomber without stock lock');results.push('ceiling collision and bounded recovery');
  k=setup();charge(k,100);g.pause();assert(!k.invader.charging&&k.stock===3,'pause cancels charge');g.resume();k.fireInput(1/120,false);assert(!k.invader.craft,'resume does not launch');g.setThinking(0);g.setThinking(2);charge(k,100);g.setThinking(0);assert(!k.invader.charging&&k.stock===3,'scroll cancels charge');results.push('pause and mode cancellation');
  ...
  k=setup();k.ultimate();charge(k,170);k.fireInput(1/120,false);assert(k.invader.craft.total===8,'batch bounded extra payload');ticks(k,900);assert(k.invader.dropped===8&&k.stock===3,'batch finite payload');results.push('batch eight-bomb cap');
  k=setup();charge(k,100);g.press('KeyQ');assert(!k.invader.charging&&k.stock===3,'dodge cancels charge');results.push('dodge cancels charge');
 ...
 await page.evaluate(()=>{const g=window.__qa;g.start();g.finishIntro();g.setThinking(2);g.audio.setMuted(true);g.invuln=100;const r=g.canvas.getBoundingClientRect();g.pointer.active=true;g.pointer.x=r.x+r.width*.32;g.pointer.y=r.y+r.height*.57;});

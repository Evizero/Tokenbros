...
export class PidalfKit {
 ...
 grab(){
  ...
  for(const h of bodies){if(h.mass>budget||this.g.peers.some(a=>a!==this.g&&(a.dimillianKit.paired===h.enemy&&!!h.enemy||a.pidalfKit.held.some(o=>o.body===h.body||!!h.grenade&&o.grenade===h.grenade)||!!h.enemy&&(a.pidalfKit.controls(h.enemy)||a.marcusKit.pacman.controls(h.enemy)||a.theoKit.controls(h.enemy)))))continue;budget-=h.mass;if(h.crate)h.body=this.loosenCrate(h.body);this.held.push({body:h.body,enemy:h.enemy,barrel:h.barrel,grenade:h.grenade,mass:h.mass,ox:clamp(h.body.x+h.body.w/2-p.x,-55,55),oy:clamp(h.body.y+h.body.h/2-p.y,-45,45),vx:0,vy:0});if(h.barrel)h.barrel.mobile=true;if(h.enemy){h.enemy.wind=0;h.enemy.dash=0;h.enemy.stun=.2;}}

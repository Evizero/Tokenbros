...
pause(){if(this.coop.pause())return;this.theoKit.wasFiring=false;this.vertical.cancel();this.marcusKit.lcd.cancel();this.marcusKit.wasFiring=false;this.marcusKit.invader.cancel();this.marcusKit.invader.wasFiring=false;this.pidalfKit.release(false);this.pidalfKit.cancelDefense();this.defense.release();this.dimillianKit.releaseDefense();this.dimillianKit.cancelLightning();this.audio.silenceRobots();this.state='paused';this.keys.clear();this.pointer.down=false;this.pendingShot=0;this.panel('PAUSED','','RESUME',()=>this.resume(),'<button class="menu-action" id="switch-bro"><span>SWITCH BRO<small>CHOOSE YOUR CHARACTER</small></span><b>⇄</b></button><button class="menu-action" id="restart"><span>RESTART MISSION<small>START A FRESH RUN</small></span><b>↻</b></button>');$('#restart').addEventListener('click',()=>this.start());$('#switch-bro').addEventListener('click',()=>showRoster(this));}

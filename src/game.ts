...
constructor(canvas:HTMLCanvasElement,preview=false){
    ...
    canvas.addEventListener('pointerdown',e=>{if((e.button!==0&&!(e.button===2&&e.ctrlKey))||e.pointerType==='touch'||this.state!=='playing'||this.coop.menu)return;e.preventDefault();point(e);canvas.focus();canvas.setPointerCapture(e.pointerId);this.pointer.down=true;this.pendingShot=.14;this.audio.start();});
...
selectCharacter(character:Game['character']){this.character=character;if(!this.preview)document.documentElement.dataset.character=character;}
  press(key:string){
...
start(){
    ...
    this.vertical.clear();this.barks.reset();this.pidalfKit.clear();this.pidalfKit=new PidalfKit(this);this.marcusKit=new MarcusKit(this);this.theoKit.clear();this.theoKit=new TheoKit(this);
...
showIntro(){
    ...
    if(this.character==='theo'){o.innerHTML=`<div class="reveal-speedlines"></div><img class="reveal-portrait" src="${import.meta.env.BASE_URL}assets/theo-intro-v2.png" alt="Theo with his skateboard"><div class="reveal-band"><div class="reveal-kicker">ONE BOARD. KEEP THE LINE.</div><div class="reveal-name">THEO<span>THE FREE RIDER</span></div><div class="reveal-bark">DID YOU GET THAT ON TAPE?</div></div><button class="skip-intro">SPACE / SKIP INTRO ↗</button>`;o.removeAttribute('hidden');$('.skip-intro').addEventListener('click',()=>this.finishIntro());this.audio.tone(180,.4,'triangle',.05,800);return;}
...
finishIntro(){if(this.state!=='intro')return;this.state='playing';this.barks.request('spawn');this.keys.clear();this.pointer.down=false;this.pendingShot=0;$('#overlay').className='overlay';$('#overlay').setAttribute('hidden','');this.canvas.focus();if(this.character==='theo'){this.notify('KEEP IT ROLLING.','Run to skate · click deck combo · E throw / recall · Q shield toss · Space air kickflip.');return;}if(this.character==='marcus'){this.notify('ANOTHER ANGLE.','Click launches · E recalls · scroll augments · Q flips · F batches.');return;}if(this.character==='pidalf'){this.notify('LESS IS MORE.','Hold E to grab · click while holding to compact · release E to throw · F places compact.');return;}if(this.character==='dimillian'){this.notify('HOT RELOAD.','Scroll forms · click attacks · E pairs bots · Q defends · F goes big.');return;}this.notify(this.character==='peter'?'THE CLAW IS THE LAW.':'POINT. SHOOT. THINK BIGGER.',this.character==='peter'?'Click attacks + commands · E throws claws · scroll changes type · F molts.':'Mouse aims · click fires · scroll ↑ increases thinking · F throws RESET.');}
pause(){if(this.coop.pause())return;this.theoKit.wasFiring=false;this.vertical.cancel();this.marcusKit.lcd.cancel();this.marcusKit.wasFiring=false;this.marcusKit.invader.cancel();this.marcusKit.invader.wasFiring=false;this.pidalfKit.release(false);this.pidalfKit.cancelDefense();this.defense.release();this.dimillianKit.releaseDefense();this.dimillianKit.cancelLightning();this.audio.silenceRobots();this.state='paused';this.keys.clear();this.pointer.down=false;this.pendingShot=0;this.panel('PAUSED','','RESUME',()=>this.resume(),'<button class="menu-action" id="restart"><span>RESTART MISSION<small>START A FRESH RUN</small></span><b>↻</b></button>', '<button class="menu-action" id="main-menu"><span>MAIN MENU<small>RETURN TO CHARACTER SELECT</small></span><b>↩</b></button>');$('#restart').addEventListener('click',()=>this.start());$('#main-menu').addEventListener('click',()=>showRoster(this));}
...
panel(title:string,body:string,button:string,action:()=>void,extra='',afterOptions=''){
    const o=$('#overlay');o.className='overlay game-menu';o.innerHTML=`<div class="briefing"><div class="result-title">${title}</div>${body?`<div class="result-stats">${body}</div>`:''}<div class="pause-actions"><button class="primary" id="panel-action">${button} <span class="arrow">▶</span></button>${extra}<button class="menu-action" id="panel-options"><span>OPTIONS<small>SOUND · DISPLAY · CONTROLS</small></span><b>☰</b></button>${afterOptions}</div></div>`;o.removeAttribute('hidden');$('#panel-action').addEventListener('click',action);$('#panel-options').addEventListener('click',()=>$('#open-options').click());$('#panel-action').focus({preventScroll:true});
  }
die(){
  const was=this.state;super.die();
  if(this.preview||this.coop.menu||was==='dead'||this.state!=='dead')return;
  this.clearToast();
  const o=$('#overlay');o.className='overlay death-screen';
...
hud(){
    ...
    $('.thinking-control').hidden=this.character==='theo';$('#control-scroll').parentElement!.hidden=this.character==='theo';$('#control-f').parentElement!.hidden=this.character==='theo';$('.reset-label').hidden=this.character==='theo';
...
drawThinkingFeedback(){
    if(this.character==='theo'||this.character==='pidalf'||this.state!=='playing'||this.modeFeedback<=0)return;
